...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        if gt_imgs is not None and render_factor==0:
            ...
            ssim_val = ssim(rgb_tensor, gt_tensor, data_range=1.0)

            print(f"SSIM: {ssim_val.item():.4f}")
            with open(os.path.join(savedir, 'ssim.txt'), 'a') as f:
                f.write(f"pic {i}: {ssim_val.item():.4f}\n")
            ...
            lpips_dist_alex = loss_fn_alex(rgb_tensor, gt_tensor)
            lpips_dist_vgg = loss_fn_vgg(rgb_tensor, gt_tensor)

            print(f"LPIPS (Alex): {lpips_dist_alex.item():.4f}")
            print(f"LPIPS (VGG): {lpips_dist_vgg.item():.4f}")
            with open(os.path.join(savedir, 'lpips_alex.txt'), 'a') as f:
                f.write(f"pic {i}: {lpips_dist_alex.item():.4f}\n")
            with open(os.path.join(savedir, 'lpips_vgg.txt'), 'a') as f:
...
def create_nerf(args):
    ...
    skips = [3]
    model = NeRF(D=args.netdepth, W=args.netwidth, r=args.rank,
    ...
    if args.N_importance > 0:
        model_fine = NeRF(D=args.netdepth_fine, W=args.netwidth_fine, r=args.rank_fine,
                          input_ch=input_ch, output_ch=output_ch, skips=skips,
                          input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs).to(device)
    ...
    if len(ckpts) > 0 and not args.no_reload:
        ...
        # optimizer.load_state_dict(ckpt['optimizer_state_dict'])  # skip optimizer when inference only

        # Load model
        # model.load_state_dict(ckpt['network_fn_state_dict'])
        loaded_state_dict = ckpt['network_fn_state_dict']  # for tp.prune model loading
...
def config_parser():
    ...
    parser.add_argument("--pruningratio", type=float, default=0.9,
                        help='pruning ratio, applied to network_fn')
...
def train():
    ...
    pruner = tp.pruner.MagnitudePruner(model=render_kwargs_train['network_fn'],
                                       ignored_layers=ignored_layers,
                                       pruning_ratio=args.pruningratio,
                                       example_inputs=torch.rand(1, 198),
                                       iterative_steps = 200000,
                                       importance=imp)
