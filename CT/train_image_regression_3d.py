...
#                                        example_inputs=torch.rand(1, 512),
#                                        iterative_steps = config['max_iter'],
#                                        importance=imp)
        ...
        if (iterations + 1) % config['val_iter'] == 0:
            ...
            with torch.no_grad():
                ...
                if test_psnr > best_psnr:
                    ...
                    # Save best model
                    model_name = os.path.join(checkpoint_directory, 'model_best.pt')
                    torch.save({'net': model.state_dict(), \
                                'optim': optim.state_dict(), \
                                    }, model_name)
                    # save full model for pnnx loading
                    torch.save(model, os.path.join(checkpoint_directory, 'model_best_whole.pt'))
...
with open(os.path.join(output_directory, 'best_psnr.txt'), 'w') as f:
