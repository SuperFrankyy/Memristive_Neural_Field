...
# set the device to cuda if available, device number is 1
...
if not(config['encoder']['embedding'] == 'none'):
    model_name += '_scale{}_size{}'.format(config['encoder']['scale'], config['encoder']['embedding_size']) # 名称中需要prune的话需要加一下
print(model_name)

train_writer = tensorboardX.SummaryWriter(os.path.join(opts.output_path + "/logs", model_name))
output_directory = os.path.join(opts.output_path + "/3dHPOtest", model_name)
...
# 设置用于超参搜索的nni的参数
netparams = config['net']
optimized_params = nni.get_next_parameter()
netparams.update(optimized_params)
print(netparams)

# Setup model
if config['model'] == 'SIREN':
    model = SIREN(netparams)
...
# qbits = config['qbits']
# target_weights = gen_target_weights(model, 'fcnet')
# bits = [qbits] * len(target_weights)
# LQ = lqnet.learned_quant(target_weights, b = bits, needbias = False)
...
# prune setting
# prune_rate = netparams['prune_rate']
# for layer in model.model:
#     if hasattr(layer, 'linear'):
#         prune.l1_unstructured(layer.linear, name='weight', amount=prune_rate)


for it, (grid, image) in enumerate(data_loader):
    # 判断是否超过了budget
    budget = 16000 # 计算的方式为256K权重/16比特，以保证一块芯片上可以放下
    rram_num = (64*131 + 64*netparams['rank']*2 + 64*1) # * (1-prune_rate)
    if rram_num > budget:
        break
    ...
    for iterations in range(max_iter):
        ...
        if (iterations + 1) % config['val_iter'] == 0:
            ...
            with torch.no_grad():
                ...
                # 设置评价指标 2 * w * log(psnr) - (1-w) * log(rram_num), w = 0.8
                score = 2 * 0.8 * np.log(test_psnr) - 0.2 * np.log(rram_num)
                metric = {'default': score, 'psnr': test_psnr, 'rram_num': rram_num}
                ...
                if test_psnr > best_psnr:
                    ...
                    # Save best model
                    model_name = os.path.join(checkpoint_directory, 'model_best_r{}.pt'.format(netparams['rank']))
...
# # save the best psnr in txt file
with open(os.path.join(output_directory, 'best_psnr.txt'), 'a') as f:
    # f.write('best_psnr: {:.4g}dB, best_iter: {}'.format(best_psnr, best_iter))
    f.write('model rank: {}, best_psnr: {:.4g}dB, best_iter: {}, best_metric: {}'.format(netparams['rank'], best_psnr, best_iter, best_metric))
    f.write('\n')
print('best_psnr: {:.4g}dB, best_iter: {}'.format(best_psnr, best_iter))

# # 保存prune后的模型
# for layer in model.model:
#     if hasattr(layer, 'linear'):
#         prune.remove(layer.linear, name='weight')

torch.save({'net': model.state_dict(), \
            'enc': encoder.B, \
            'opt': optim.state_dict(), \
            }, os.path.join(checkpoint_directory, 'model_r{}.pt'.format(netparams['rank'])))
